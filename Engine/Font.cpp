...
void Font::DrawWords(std::string text, Graphics& gfx , CoordinateTransformer& ct , Vec2I pos)
{
	for (auto& c : text)
	{
		if (c >= FirstLetter && c <= LastLetter)
		{
			...
			gfx.DrawSprite(ct.Transform(pos), SpriteSheet, Letter, Graphics::GetScreenRect(), e);
