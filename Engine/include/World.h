...
public:
	...
	World(Sprite& sprite)
	{
		...
		Elements = std::vector<Element>(width * height, Element(RectI(ElemSize, ElemSize, Vec2I(0, 0))));
	...
	Element* GetElem(const Vec2I& pos)
	{
	...
	Vec2I ScreenToMatrixPos(const Vec2I& pos) const
	{
	...
	int MatrixPosToIndex(const Vec2I& pos) const
	{
		return pos.y * SandboxDim.width + pos.x;
	}
	int GetElemIndex(const Vec2I& pos) const
	{
		return pos.y * SandboxDim.width + pos.x;
	}
	int GetElemIndScr(const Vec2I& screenpos) const
	{
...
protected:
	...
	Sprite BackGround = Sprite(std::string("assets/background.bmp"));
};

struct MoveableElement {
	MoveableElement(const Element& element, Vec2D vel0)
	{
