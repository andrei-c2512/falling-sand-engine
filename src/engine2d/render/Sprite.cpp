...
namespace e2d {
	...
	Sprite::Sprite(const Size<short>& dim, chili::Color c)
	{
	...
	{
		std::string str = std::string("Sprite: ") + std::filesystem::current_path().string() + '\n';
		...
		for (int y = StartY; y != EndY; y += dy)
		{
			...
			{
				PutPixel(x, y, chili::Color(file.get(), file.get(), file.get()));
				if (is32bit)
				{
	...
	void Sprite::ScaleSprite(float scale)
	{
		...
		for (float y = 0; y < height; y += scale)
		{
			...
			{
				chili::Color c = old.GetPixel(x / scale, y / scale);
	...
	void Sprite::PutPixel(int x, int y, chili::Color c)
	{
		pColor[(y * width) + x] = c;
	}

	chili::Color Sprite::GetPixel(int x, int y) const
	{
	...
	Sprite::Sprite(const Size<short>& dim, const std::vector<chili::Color>& pcolor)
	{
		pColor = pcolor;
		width = dim.width;
		height = dim.height;
	...
	void Sprite::Merge(const Sprite& sprite)
	{
		...
		for (size_t ind = 0; ind < width * height; ind++)
		{
			...
			pColor[ind] = chili::Color(r, g, b);
		}
	}

	void Sprite::Merge(const chili::Color* sprite, int width0, int height0)
	{
