#include "Creator.hpp"
#include "assert.h"
Creator::Creator(const RectI& ButtonSize, int Radius , Simulation& world , Weather& weather0)
	:world(world), Chance(1 , 100),weather(weather0),explosion(Explosion(world))
{
	...
	for (size_t i = 0; i < (int)WeatherType::Count; i++)
	{
		...
		WButtons.emplace_back(WeatherButton(Vec2I( x , WeatherButtonPos.y ), WeatherType(i)));
	}
