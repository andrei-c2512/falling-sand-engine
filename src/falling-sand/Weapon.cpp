...
namespace sand {
	Weapon::Weapon(const e2d::Rect* owner, int damage0, float timer)
	...
	RangedWp::RangedWp(const e2d::Rect* owner, int damage, float speed0, int MaxRounds0, float rTime, float sTime)
		:proj_speed(speed0),
	...
	bool RangedWp::Collision(const e2d::Rect& rect)
	{
	...
	void RangedWp::Use(const e2d::Vec2D& target_pos)
	{
		if (CanShoot())
		{
			assert(Rounds > 0);
			e2d::Vec2D start_pos = { pOwner->left , pOwner->bottom };
			e2d::Vec2D vel = { target_pos.x - start_pos.x , target_pos.y - start_pos.y };
	...
	void RangedWp::DrawProjectiles(e2d::Renderer& gfx, const e2d::Camera& ct, chili::Color c)
	{
		for (auto& proj : proj_list)
		{
	...
	void RangedWp::Draw(e2d::Renderer& gfx, e2d::Camera& ct)
	{
		DrawProjectiles(gfx, ct, chili::Colors::Red);
	}
	...
	const e2d::Rect* Weapon::GetpOwner() const {
		return pOwner;
	}
	...
	void ExplosiveLauncher::LaunchNewProj(const e2d::Vec2D& vel, const e2d::Vec2D& initpos)
	{
	...
	std::forward_list<e2d::Rect> RangedWp::GetWeaponHitBoxes() const
	{
		std::forward_list<e2d::Rect> HitBox_list;
		for (auto& proj : proj_list)
		{
