...
#include "falling-sand/Attributes.hpp"
...
namespace sand {
	...
	public:
		World()
		{
			...
			Elements = std::vector<Element>(width * height, Element(e2d::RectI(ElemSize, ElemSize, e2d::Vec2I(NULL, NULL))));

			for (int y = 0; y < height; y++)
			{
				for (int x = 0; x < width; x++)
				{
					e2d::Vec2D Pos = e2d::Vec2D(float(x * ElemSize) + e2d::Renderer::WorldArea.left, float(y * ElemSize) + e2d::Renderer::WorldArea.bottom);
		...
		World(const e2d::Sprite& sprite)
		{

			const int width = int(SandboxDim.width);
			const int height = int(SandboxDim.height);

			Elements = std::vector<Element>(width * height, Element(e2d::RectI(ElemSize, ElemSize, e2d::Vec2I(0, 0))));
			...
			for (int y = 0; y < height; y++)
			{
				for (int x = 0; x < width; x++)
				{
					...
					chili::Color c = sprite.GetPixel((x * ElemSize), (y * ElemSize));

					e2d::Vec2D Pos = e2d::Vec2D(float(x * ElemSize), float(y * ElemSize));
		...
		Type ConvertPixel(chili::Color c)
		{
		...
		virtual void DrawWorld(e2d::Renderer& gfx, const e2d::Camera& ct)
		{
		...
		static constexpr e2d::Size<int> SandboxDim = e2d::Size<int>(e2d::Renderer::WorldArea.width / ElemSize,
			e2d::Renderer::WorldArea.height / ElemSize);
		...
		e2d::Size<int> GetSandboxDim() const
		{
			return SandboxDim;
		}
		...
		void AddToFireList(const Move& move)
		{
			Fire_list.emplace_back(std::move(move));
		}
		void AddToFireAuraList(const Move& move)
		{
			FireAura_list.emplace_back(std::move(move));
		}
		void AddToConversionList(const Move& move)
		{
			Conversion_list.emplace_back(std::move(move));
		}
		void AddToSpawnList(const Spawn& move)
		{
		...
		Element* GetElem(const e2d::Vec2I& pos)
		{
			return &Elements[(pos.y) * SandboxDim.width + pos.x];
		}

		e2d::Vec2I IndexToPos(int index)
		{
			return e2d::Vec2I(int(index % SandboxDim.width), int(index / SandboxDim.width));
		}
		e2d::Vec2I ScreenToMatrixPos(const e2d::Vec2I& pos) const
		{
			//adding half of the sandbox dimensions because the matrix goes equaly in the - and + directions
			// 
			return e2d::Vec2I(pos.x / ElemSize + SandboxDim.width / 2, pos.y / ElemSize + SandboxDim.height / 2);
		}
		int MatrixPosToIndex(const e2d::Vec2I& pos) const
		{
			return pos.y * SandboxDim.width + pos.x;
		}
		int GetElemIndex(const e2d::Vec2I& pos) const
		{
			return pos.y * SandboxDim.width + pos.x;
		}
		int GetElemIndScr(const e2d::Vec2I& screenpos) const
		{
	...
	protected:
		...
		e2d::Sprite BackGround = e2d::Sprite(std::string("assets/background.bmp"));
	};

	struct MoveableElement {
		MoveableElement(const Element& element, e2d::Vec2D vel0)
		{
			atr.GetAttributes(element);
			e2d::RectI elem_hitbox = element.GetRect();
			HitBox = e2d::Rect(elem_hitbox.width, elem_hitbox.height, e2d::Vec2D(elem_hitbox.left, elem_hitbox.bottom));
			vel = std::move(vel0);
		...
		{
			gfx.DrawRect(ct.TransformX(HitBox.left), ct.TransformY(HitBox.bottom), HitBox.width, HitBox.height, chili::Colors::Magenta);
		}
		e2d::Rect HitBox;
		Attributes atr;
		e2d::Vec2D vel;
	...
	public:
		void Go(float time)
		{
			for (auto& elem : particle_list)
			{
				...
				e2d::Vec2D& vel = elem.vel;
				e2d::Vec2D pos = elem.HitBox.GetPos();
				...
				e2d::Vec2D new_pos = elem.HitBox.GetPos();
		...
		void MoveRight(MoveableElement& elem, float time, float vel)
		{
			...
			while (AddX)
			{
				if (LineX >= e2d::Renderer::WorldArea.right())
				{
					break;
				}
				bool Move = true;
				...
				{
					e2d::Vec2I pos = ScreenToMatrixPos(e2d::Vec2I(LineX, y));
		...
		void MoveLeft(MoveableElement& elem, float time, float vel)
		{
			...
			while (AddX)
			{
				if (LineX < e2d::Renderer::WorldArea.left)
				{
					break;
				}
				bool Move = true;
		...
		void MoveUp(MoveableElement& elem, float time, float vel)
		{
			...
			while (AddY)
			{
				if (LineY >= e2d::Renderer::WorldArea.top())
				{
					break;
				}
				bool Move = true;
				...
				{
					e2d::Vec2I pos = ScreenToMatrixPos(e2d::Vec2I(x, LineY));
					int index = pos.y * World::SandboxDim.width + pos.x;
					elem_list.emplace_back(GetElem(index)->GetState());
				}
		...
		void MoveDown(MoveableElement& elem, float time, float vel)
		{
			...
			while (AddY)
			{
				if (LineY < e2d::Renderer::WorldArea.bottom)
				{
					break;
				}
				bool Move = true;
		...
		void PutIntoMatrix(const MoveableElement& elem)
		{
			...
			e2d::Vec2I scr_pos = e2d::Vec2I(std::move(pos.x), std::move(pos.y));
			e2d::Vec2I matrix_pos = ScreenToMatrixPos(std::move(scr_pos));
		...
		void DrawWorld(e2d::Renderer& gfx, const e2d::Camera& ct) override {
