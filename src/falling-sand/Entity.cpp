...
namespace sand {
	Entity::Entity(e2d::Rect rect, e2d::Sprite sprite0, float dmg, int health, float speed)
		:HitBox(rect), sprite(sprite0), Damage(dmg), Speed(speed)
	{
		...
		hp_bar.dim = e2d::Size<short>((int)HitBox.width + 2 * HpBar::offset, (int)HitBox.height / 5);
	...
	void Entity::Draw(e2d::Renderer& gfx, const e2d::Camera& cam)
	{
		...
		e2d::Vec2I pos = cam.Transform(HitBox.GetPos());

		gfx.DrawRect(HitBox.left, HitBox.bottom, HitBox.width, HitBox.height, chili::Colors::Red);
		...
		if (hp_bar.show_timer.IsReady() == false)
		{
			...
			if (cnt < limit / 2)
			{
				gfx.DrawRect(x, y, bar_width, hp_bar.dim.height, chili::Colors::Green);
				//gfx.DrawRect_Border(x, y, bar_width, hp_bar.dim.height, chili::Colors::White);
			}
			else
			{
				float elapsed_time = (cnt - limit / 2);
				unsigned char transparency = 255 - ((elapsed_time / (limit / 2))) * 255.0f;

				auto c = chili::Colors::Green;
				c.SetA(transparency);
				gfx.DrawRectA(x, y, bar_width, hp_bar.dim.height, c);
	...
	void Entity::MoveX(World& world, float time)
	{
		...
		if (vel.x > 0.0f)
		{
			ZoneX = e2d::RectI(Size, int(HitBox.height), e2d::Vec2I(int(HitBox.right()), int(HitBox.bottom)));
		}
		else if (vel.x <= 0.0f)
		{
			ZoneX = e2d::RectI(Size, int(HitBox.height), e2d::Vec2I(int(HitBox.left - Size), int(HitBox.bottom)));
		}

		int StartX, EndX, Add;
		std::function<bool(int, int)> Condition;
		std::function<float(e2d::Rect, e2d::RectI)> dist;
		if (vel.x > 0.0f)
		{
			StartX = ZoneX.left;
			EndX = ZoneX.right();
			...
			dist = [](e2d::Rect rect, e2d::RectI elem)
				{
		...
		auto dim = world.GetSandboxDim();
		{
			...
			for (int x = StartX; Condition(x, EndX); x += Add)
			{
				...
				for (int y = ZoneX.bottom; y < ZoneX.top(); y += World::ElemSize)
				{
					...
					size_t ind = world.GetElemIndScr(e2d::Vec2I(x, y));
	...
	void Entity::MoveY(World& world, float time)
	{
		...
		e2d::RectI ZoneY, PortrustionZoneY;

		if (vel.y >= 0.0f)
		{
			ZoneY = e2d::RectI(HitBox.width, Size, e2d::Vec2I(HitBox.left, HitBox.top()));
		}
		else if (vel.y < 0.0f)
		{
			ZoneY = e2d::RectI(HitBox.width, Size, e2d::Vec2I(HitBox.left, HitBox.bottom - Size));
		}
		auto dim = world.GetSandboxDim();
		...
		if (vel.y >= 0.0f)
		{
			...
			Condition = [=](int nr1, int nr2)
				{
					return nr1 < nr2 && nr1 + World::ElemSize < e2d::Renderer::ScreenHeight;
				};
			GetEdge = [=]()
				{
					return HitBox.top();
	...
	void Entity::SetPos(e2d::Vec2I& pos)
	{
		HitBox.SetPos(e2d::Vec2D(pos.x, pos.y));
	}

	e2d::Rect Entity::GetRect() const
	{
		return HitBox;
	}
	...
	void Entity::MoveRight(World& world, float time)
	{
		...
		while (AddX)
		{
			...
			{
				e2d::Vec2I pos = world.ScreenToMatrixPos(e2d::Vec2I(LineX, y));
	...
	void Entity::MoveUp(World& world, float time)
	{
		...
		while (AddY)
		{
			...
			{
				e2d::Vec2I pos = world.ScreenToMatrixPos(e2d::Vec2I(x, LineY));
				int index = pos.y * World::SandboxDim.width + pos.x;
				elem_list.emplace_back(world.GetElem(index)->GetState());
			}
	...
	void Entity::Kill(World& world)
	{
		...
		for (int y = HitBox.bottom; y < HitBox.top(); y++)
		{
			...
			{
				e2d::Vec2I pos = world.ScreenToMatrixPos(e2d::Vec2I(x, y));
				int index = pos.y * World::SandboxDim.width + pos.x;
