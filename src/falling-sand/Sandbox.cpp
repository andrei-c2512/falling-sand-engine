...
namespace sand {
	Sandbox::Sandbox(chili::Mouse& mouse, ParticleEffect& effect, e2d::Camera& camera)
		:weather(simulation, WeatherType::Clear, size_t(2)), mob_list(simulation, effect, mouse, camera)
	{
		{
			...
			for (int y = 0; y < height; y += ChunkSize)
			{
				for (int x = 0; x < width; x += ChunkSize)
				{
					...
					Chunk_list.emplace_back(e2d::RectI(CWidth, CHeight, e2d::Vec2I(x, y)), simulation, index);
	...
	void Sandbox::UpdateSandbox(const chili::Mouse& mouse, const chili::Keyboard& kbd, float time)
	{
	...
	void Sandbox::DrawSandbox(e2d::Renderer& gfx, const e2d::Camera& cam, const chili::Mouse& mouse)
	{
	...
	void Sandbox::DrawChunkBorders(e2d::Renderer& gfx, const e2d::Camera& cam)
	{
	...
	size_t Sandbox::GetChunkIndex(size_t elemindex)
	{
		...
		e2d::Vec2I ElemPos(elemindex % dim.width, elemindex / dim.width);
		e2d::Vec2I ChunkPos(ElemPos.x / ChunkSize, ElemPos.y / ChunkSize);
