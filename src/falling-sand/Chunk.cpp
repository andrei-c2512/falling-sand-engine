#include "falling-sand/Chunk.hpp"
...
namespace sand {
				...
				for (int x = StartX; x != EndX; x += AddX)
				{
					...
					if (world.GetElem(index)->IsUpdated() == false)
					{
						...
						case Type::Fire:
							...
							IsActive = true;
							if (e2d::RNG::RandomPercentile() <= 30)
							{
						...
						case Type::Acid:
							...
							if (e2d::RNG::RandomPercentile() > 50)
								Update_Acid(index);
							break;
						case Type::ToxicGas:
	...
	std::pair<bool, Chunk::NextMove> Chunk::IsElemAtBorder(int index)
	{
		...
		auto pos = e2d::Vec2I(index % dim.width, index / dim.width);
	...
	*/
	...
	{
		auto dim = world.GetSandboxDim();
		const auto pos = e2d::Vec2I(int(index / dim.width), int(index % dim.width));
	...
	int Chunk::GetDelta(int index, Direction dir, e2d::Size<int> dim) {
		int Delta = 0;
	...
	void Chunk::Update_Acid(int index)
	{
		...
		for (int y = -1; y <= 1; y++)
		{
			for (int x = -1; x <= 1; x++)
			{
				...
				if (newind >= 0 && newind < dim.GetArea())
				{
					...
					{
						if (e2d::RNG::RandomPercentile() <= Element::AcidDestroyChance)
							Pos_list.emplace_back(newind);
					}
				}
			}
		...
		if (Pos_list.size() != 0)
		{
			...
			int WorldIndex = Pos_list[RandSpread.GetVal()];
			Type type;
			if (e2d::RNG::RandomPercentile() > 60)
				type = Type::Empty;
			else
				type = Type::ToxicGas;
